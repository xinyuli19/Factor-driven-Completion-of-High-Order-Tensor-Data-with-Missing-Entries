...
```{r}
MRE<-function(X_hat,X,rate){
  ind<-(abs(X)<10^(-5))    #X元素为0的下标
  dimension<-dim(X)[1]*dim(X)[2]*dim(X)[3]
  missnum<-floor(dimension*rate)
  excludenum<-sum(ind)   #不考虑这些原先矩阵元素值为0的元素
  #通过把这些下标所在的元素都置为0.001来避免分母为0的NaN问题
  X_hat[ind]<-0.001
  X[ind]<-0.001
  
  return(sum(abs(X_hat-X)/abs(X))/(missnum-excludenum))
}
```
...
#初始值设定
repeats<-100 
mre<-matrix(rep(0,12),nrow=3,byrow=TRUE)
...
for(number in 1:repeats){
  ...
  #计算误差指标
  mre[1,1]<-mre[1,1]+MRE(X_hat,X_complete,0.025)
  mre[1,2]<-mre[1,2]+MRE(X1,X_complete,0.025)
  mre[1,3]<-mre[1,3]+MRE(X2,X_complete,0.025)
  mre[1,4]<-mre[1,4]+MRE(X3,X_complete,0.025)
}
...
for(number in 1:repeats){
  ...
  #计算误差指标
  mre[2,1]<-mre[2,1]+MRE(X_hat,X_complete,0.05)
  mre[2,2]<-mre[2,2]+MRE(X1,X_complete,0.05)
  mre[2,3]<-mre[2,3]+MRE(X2,X_complete,0.05)
  mre[2,4]<-mre[2,4]+MRE(X3,X_complete,0.05)
}
...
for(number in 1:repeats){
  ...
  #计算误差指标
  mre[3,1]<-mre[3,1]+MRE(X_hat,X_complete,0.075)
  mre[3,2]<-mre[3,2]+MRE(X1,X_complete,0.075)
  mre[3,3]<-mre[3,3]+MRE(X2,X_complete,0.075)
  mre[3,4]<-mre[3,4]+MRE(X3,X_complete,0.075)
}

mre<-round(mre/repeats,4)
evaluation[1,3:6]<-mre[1,1:4]
evaluation[2,3:6]<-mre[2,1:4]
evaluation[3,3:6]<-mre[3,1:4]
cat("插补后的补全矩阵和原矩阵间MRE为","\n")
print(evaluation)
```
